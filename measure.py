...
trig_arr = [15,13,32,38,36] #Trigピン番号(正面、左、右)
echo_arr = [26,24,31,37,35] #Echoピン番号(正面、左、右)
...
prev_time = time.time()
if environment == 1:
    def Log(text, d_fr, d_lh, d_rh, d_ls, d_rs, prev_d_fr, prev_d_lh, prev_d_rh, b_ls, b_rs):
        now_time = time.time()
        global prev_time
        if  Comp(d_fr, prev_d_fr) or Comp(d_lh, prev_d_lh) or Comp(d_rh, prev_d_rh) or Comp(d_ls, b_ls) or Comp(d_rs, b_rs):
            print('{:.1f} 秒'.format(now_time - logTime))
            print('{:.3f} 秒\n'.format(now_time - prev_time))
            print('右の距離   {:>4.1f} cm'.format(d_rs)) #距離を表示する
            print('右前の距離 {:>4.1f} cm'.format(d_rh)) #距離を表示する
            print('正面の距離 {:>4.1f} cm'.format(d_fr)) #距離を表示する
            print('左前の距離 {:>4.1f} cm'.format(d_lh)) #距離を表示
            print('左の距離   {:>4.1f} cm'.format(d_ls)) #距離を表示
            print(text)
            print("\033[A\033[A\033[A\033[A\033[A\033[A\033[A\033[A\033[A", end='\r')
            prev_time = now_time
else:
    def Log(text, d_fr, d_lh, d_rh, d_ls, d_rs, prev_d_fr, prev_d_lh, prev_d_rh, b_ls, b_rs):
        return
...
while True:  #以下の部分をずっと繰り返す
    try:
        ...
        d_ls = Measure(trig_arr[LEFT_SIDE_SENSOR],echo_arr[LEFT_SIDE_SENSOR]) 
        d_rs = Measure(trig_arr[RIGHT_SIDE_SENSOR],echo_arr[RIGHT_SIDE_SENSOR]) 

        Log("", d_fr, d_lh, d_rh, d_ls, d_rs, b_fr, b_lh, b_rh, b_ls, b_rs)
        b_fr = d_fr
        b_lh = d_lh
        b_rh = d_rh
        b_ls = d_ls
