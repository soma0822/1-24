...
echo_arr = [26,24,31,37, 35] #Echoピン番号(正面、左、右,左サイド)
...
if environment == 1:
    def Log(text, d_fr, d_lh, d_rh, d_lsh):
        global ftext
        if True:
            ftext = text
            try:
                ...
                f.write('左サイドのセンサー {:.1f} cm\n'.format(d_lsh)) #距離を表示する
                # print(text)
                f.write(text + "\n")
...
time_of_starting_to_turn_r = 0
time_of_starting_to_turn_l = 0

def Cntl(d_fr, d_lh, d_rh, d_lsh):
    global time_of_starting_to_turn_r
    global time_of_starting_to_turn_l
    if d_fr < 20:
        Log('判断：止まる', d_fr, d_lh, d_rh, d_lsh)
        # 前輪をまっすぐ向ける
        pwm.set_pwm(SERVO, 0, PWM_STRAIGHT)
        # タイヤを停止させる
        pwm.set_pwm(SPEED, 0, PWM_STOP)
        ...
        if d_lsh < 30:
            if 0 < time_of_starting_to_turn_l:
                time_of_starting_to_turn_l = 0
            if time_of_starting_to_turn_r == 0.0:
                time_of_starting_to_turn_r = time.time()
            Log('右に曲がる', d_fr, d_lh, d_rh, d_lsh)
            # 前輪を右に向ける
            pwm.set_pwm(SERVO, 0, PWM_RIGHT)
            # タイヤを前進方向に回転させる
            if d_fr < 70:
                pwm.set_pwm(SPEED, 0, PWM_FORWARD_MIN)
            elif time.time() - time_of_starting_to_turn_r < CORNERING_TIME :
                pwm.set_pwm(SPEED, 0, PWM_FORWARD_MID)
            else :
                pwm.set_pwm(SPEED, 0, PWM_FORWARD_MIN)
        elif d_lh > 140 or d_rh < 30:
            if 0 < time_of_starting_to_turn_r:
                time_of_starting_to_turn_r = 0
            if time_of_starting_to_turn_l == 0.0:
                time_of_starting_to_turn_l = time.time()
            Log('左に曲がる', d_fr, d_lh, d_rh, d_lsh)
            # 前輪を左に向ける
            pwm.set_pwm(SERVO, 0, PWM_LEFT)
            # タイヤを前進方向に回転させる
            if d_fr < 70:
                pwm.set_pwm(SPEED, 0, PWM_FORWARD_MIN)
            elif time.time() - time_of_starting_to_turn_l < CORNERING_TIME :
                pwm.set_pwm(SPEED, 0, PWM_FORWARD_MID)
            else :
                pwm.set_pwm(SPEED, 0, PWM_FORWARD_MIN)
...
while True:
    try:
        ...
        if d_fr < b_fr - 150 or d_fr > b_fr + 150:
            print("=========")
            print(d_fr)
            print(b_fr)
            d_fr = Measure(trig_arr[FRONT_SENSOR], echo_arr[FRONT_SENSOR])
        ...
        if d_lh < b_lh - 150 or d_lh > b_lh + 150:
            print("=========")
            print(d_lh)
            print(b_lh)
            d_lh = Measure(trig_arr[LEFT_SENSOR], echo_arr[LEFT_SENSOR])
        ...
        if d_rh < b_rh - 150 or d_rh > b_rh + 150:
            print("=========")
            print(d_rh)
            print(b_rh)
            d_rh = Measure(trig_arr[RIGHT_SENSOR], echo_arr[RIGHT_SENSOR])
            print(d_rh)
        d_lsh = Measure(trig_arr[LEFT_SIDE_SENSOR], echo_arr[LEFT_SIDE_SENSOR])
        if d_lsh < b_lsh - 150 or d_lsh > b_lsh + 150:
            print("=========")
            print(d_lsh)
            print(b_lsh)
            d_lsh = Measure(trig_arr[LEFT_SIDE_SENSOR], echo_arr[LEFT_SIDE_SENSOR])
